...
<div class="row my-5">
    <div class="col-4"><strong>Creato:</strong> {{ $technology->getDate($technology->created_at, 'd-m-Y H:m:s') }} </div>
    <div class="col-4"><strong>Modificato:</strong> {{ $technology->getDate($technology->updated_at, 'd-m-Y H:m:s') }}</div>
    <div class="col-4">
        <div class="d-flex justify-content-end gap-3">
            <a href="{{ route('admin.technologies.index') }}" class="btn btn-primary"><i
                    class="fa-solid fa-arrow-left me-2"></i>Torna alla lista</a>
            <a href="{{ route('admin.technologies.edit', $technology->id) }}" class="btn btn-warning"><i
            ...
            <form action="{{ route('admin.technologies.destroy', $technology->id) }}" method="POST" class="delete-form"
                data-bs-toggle="modal" data-bs-target="#delete-modal" data-technology="{{ $technology->label }}">
                @csrf
                @method('DELETE')
